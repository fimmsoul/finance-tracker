...
export default function StockRow({ stock, onUpdate, onDelete, isNew }: StockRowProps) {
  ...
  return (
    <tr
      ref={setNodeRef}
      style={style}
      className="border-b border-[var(--color-border-light)] hover:bg-[var(--color-bg-sidebar)]/60 transition-colors duration-200 group"
    >
      ...
      <td className="py-1.5 px-2 w-28 text-right text-[13px] tabular-nums">
        <span className={gain >= 0 ? 'text-red-600' : 'text-blue-600'}>
        ...
        {confirming ? (
          <div className="flex gap-1">
            <button
              ...
              className="text-[11px] font-medium text-[var(--color-negative)] hover:text-red-700 cursor-pointer transition-colors duration-200"
            >
              Yes
            </button>
            <button
              onClick={() => setConfirming(false)}
              className="text-[11px] font-medium text-[var(--color-text-muted)] hover:text-[var(--color-text-secondary)] cursor-pointer transition-colors duration-200"
            >
              No
            </button>
          </div>
